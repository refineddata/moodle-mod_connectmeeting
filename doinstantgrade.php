...
if ( $courseid && isset( $CFG->connect_instant_regrade ) AND $CFG->connect_instant_regrade ) {
	if ( $connects = $DB->get_records( 'connectmeeting', array( 'course'=>$courseid ) ) ) {
		require_once( $CFG->dirroot . '/mod/connectmeeting/lib.php' );
...
if( isset( $CFG->show_instant_regrade_message ) && $CFG->show_instant_regrade_message && 
	...
	redirect( "$CFG->wwwroot/course/view.php?id=$courseid#section-$section", get_string( 'connect_grades_notyet', 'connectmeeting' ), 25 );
}else{
