...
$PAGE->set_url('/mod/connectmeeting/launch.php');
...
if ($course = $DB->get_record('course', array('id' => $courseid))) {
    if ($connectmeeting = $DB->get_record('connectmeeting', $connectquery, '*', IGNORE_MULTIPLE)) {
        if ($cm = get_coursemodule_from_instance('connectmeeting', $connectmeeting->id, $course->id)) {
        	if ( !isset( $guest ) || !$guest ) {
...
if (!$edit && !$archive && !$guest) {
    connectmeeting_launch($acurl, $courseid, true, $cm);
}
